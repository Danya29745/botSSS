...
class User(Base):
    ...
    cabinet_id      = Column(String(12), unique=True, nullable=False)   # уникальный ID кабинета
    referral_code   = Column(String(8),  unique=True, nullable=False)   # собственный реф-код
    ...
    referrals       = relationship("User", foreign_keys=[referred_by], backref="referrer_user")
...
async def get_or_create_user(
    ...
    while True:
        ...
        exists = await session.execute(select(User).where(User.cabinet_id == cabinet_id))
        if not exists.scalar_one_or_none():
            break

    while True:
        referral_code = _generate_referral_code()
        exists = await session.execute(select(User).where(User.referral_code == referral_code))
        if not exists.scalar_one_or_none():
            break
...
async def get_user_by_cabinet_id(session: AsyncSession, cabinet_id: str) -> User | None:
    result = await session.execute(select(User).where(User.cabinet_id == cabinet_id))
    return result.scalar_one_or_none()
...
async def get_new_users_since(session: AsyncSession, since: datetime) -> list[User]:
    result = await session.execute(
